...
__all__ = ('DFL', 'HGBlock', 'HGStem', 'SPP', 'SPPF', 'C1', 'C2', 'C3', 'MANet', 'C2f', 'C3x',
           'C3TR', 'C3Ghost','GhostBottleneck', 'Bottleneck', 'BottleneckCSP', 'Proto', 'RepC3', 'HyperComputeModule')
...
class HyperComputeModule(nn.Module):
    """
    Dynamic Attention–Based Hypergraph Computation with Adaptive Sparsification
    """

    def __init__(self, c1, c2, threshold):
        super().__init__()

        self.threshold = threshold  # reused as attention temperature
        self.topk_ratio = 0.25      # keeps 25% strongest edges (adaptive sparsification)

        # Linear projections for attention
        self.q_proj = nn.Linear(c1, c1, bias=False)
        self.k_proj = nn.Linear(c1, c1, bias=False)
        self.v_proj = nn.Linear(c1, c1, bias=False)
    ...
    def forward(self, x):
        ...
        x_nodes = x.view(b, c, n).transpose(1, 2).contiguous()  # [B, N, C]

        # ---- Dynamic Attention-Based Hypergraph ----
        Q = self.q_proj(x_nodes)  # [B, N, C]
        K = self.k_proj(x_nodes)  # [B, N, C]
        V = self.v_proj(x_nodes)  # [B, N, C]

        # Scaled dot-product attention (semantic similarity)
        attn = torch.matmul(Q, K.transpose(-2, -1))  # [B, N, N]
        attn = attn / (c ** 0.5)
        attn = torch.softmax(attn / self.threshold, dim=-1)

        # ---- Adaptive Sparsification (Top-K Hyperedges) ----
        ...
        x_out = self.hgconv(x_nodes, sparse_attn) + x_nodes

        # Restore spatial shape
        x_out = x_out.transpose(1, 2).contiguous().view(b, c, h, w)
